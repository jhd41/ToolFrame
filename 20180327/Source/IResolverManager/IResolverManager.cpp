...
bool IResolverManager::LocalTimeProcess( uint64 uMillSeconds /*= 0*/ )
{
	...
	while (!timeOut.IsTimeOut())
	{
		//如果都没有超时，代表处理完了
		bool bTimeOut = false;
		bTimeOut |= GetLocalTimeOutQueue()->TestTimeOut(timeOut.TimeRemaining());
		bTimeOut |= GetLocalTimeOutQueue()->ProcessTimeOut(timeOut.TimeRemaining());
