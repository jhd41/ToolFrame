...
bool MDebug::ElapsedReduce(const std::string& sTag,uint64 uTimeElapsed)
{
	if (sTag.empty())return false;
...
std::string MDebug::DebugString(const VectorString& vDebug) const
{
	...
	if (ToolFrame::IsHasValue(vDebug, "DebugPoint"))
	{
		...
		foreach(itr, _vTagCount.GetMap()) {
			char sz[1024] = {0};
			sprintf_s(sz, "Tag:%50s Count:%d", itr->first.c_str(), itr->second.GetCount());
			sStream << sz << std::endl;
		}
	}
	...
	if (ToolFrame::IsHasValue(vDebug, "DebugElapsed"))
	{
		...
		foreach(itr, _vTagElapsed.GetMap()) {
			...
			char sz[1024] = { 0 };
			sprintf_s(sz,"Tag:%20s Count:%3d Using:%3d TotalElapsed:%10lld ElapsedMin:%4lld TimeMax:%4lld ElapsedAvg:%4lld ElapsedFirst:%4lld ElapsedLast:%4lld", itr->first.c_str(), info._uCount, info._uUsing, info._uTotal, info._uTimeMin, info._uTimeMax, info._tTimeAvg.GetAvgTime(), info._uElapsedFirst,info._uElapsedLast);
			sStream << sz << std::endl;
		}
	}
...
CDebugElapsed::~CDebugElapsed()
{
	MDebug::Singleton().ElapsedReduce(_sTag, _tTimeElapse.TimeElapsed());
}
