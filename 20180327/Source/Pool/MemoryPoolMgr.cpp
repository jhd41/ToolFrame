...
std::string CMemoryPoolMgr::DebugString() const
{
	...
	foreach(itr,_vPool) {
		...
		sStream << "Size:" << itr->first<<" InvaildCount:"<< pPool->GetInvaildCount() <<" VaildCount:"<< pPool->GetVaildCount() <<" TotalCount:"<< pPool->GetTotalCount()<<" Memory:"<< ToolFrame::ValueToRead(uMemory) << std::endl;
