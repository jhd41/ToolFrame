...
bool CServerCacheManager::SyncProcessHttp(uint uCacheID, const IHttpResponse& xResponse)
{
	return SyncCallProcesser<IServerCache>(uCacheID, &IServerCache::SyncProcessHttp, xResponse);
}
...
bool CServerCacheManager::SyncCalState(uint& uTotal, uint& uVaild, uint& uNormal, uint& uQueue, uint& uDelete, uint& uPerpairing, uint& uCache) const
{
	return ISyncProcesserManager::SyncCalState(uTotal, uVaild, uNormal, uQueue, uDelete, uPerpairing, uCache);
}
