...
bool ToolFrame::HServerCacheLog::OnCacheProcessHttpErr(IServerCachePtr pCache, const IHttpResponse& xResponse)
{
	LOG_ERROR_MSG("UserID:" << pCache->GetCacheID() << " HttpResponse:" << xResponse.ToFormat());
	return true;
}
...
bool IServerCache::SyncProcessHttp(const IHttpResponse& xResponse)
{
	return GetOperateQueue().Operate(ToolFrame::Bind(&IServerCache::OnCacheProcessHttp, this), xResponse);
}
...
bool IServerCache::OnCacheProcessHttp(const IHttpResponse& xResponse)
{
	if (!_hCacheHandler->OnCacheProcessHttp(GetCachePtr(), xResponse))
		_hCacheHandler->OnCacheProcessHttpErr(GetCachePtr(), xResponse);
