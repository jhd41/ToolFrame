...
public:
	...
	virtual bool	SetQueueProcess(size_t uQueueProcess );
...
public:
	...
	//发送Http请求
	virtual bool	HttpPost(const std::string& sUrl);
	virtual bool	HttpPost(const std::string& sUrl, const MapStringString& vArg);
	virtual bool	HttpResponse(const IHttpResponse& resp);
	virtual	bool	OnHttpResponse(const IHttpResponse& resp);//子类继承
protected:
