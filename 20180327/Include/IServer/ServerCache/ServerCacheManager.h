...
public:
	...
	virtual bool SyncSave(const XTopologyID& xSrcID, uint uCacheID,void* pUserData = nullptr, int nUserInt = 0);

	virtual bool SyncKick (const XTopologyID& xSrcID, uint uCacheID, void* pUserData = nullptr, int nUserInt = 0);
	virtual bool SyncLogin(const XTopologyID& xSrcID, uint uCacheID, void* pUserData = nullptr, int nUserInt = 0);		//登录成功
	...
	virtual bool SyncProcessHttp(uint uCacheID, const IHttpResponse& xResponse);	//处理Http
