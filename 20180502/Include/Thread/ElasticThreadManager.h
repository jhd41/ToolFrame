...
public:
	...
	virtual uint64 GetRunOnceTimeAvg()const;	//单次运行时间
	virtual uint64 GetRunOnceTimeMax()const;	//单次运行时间峰值
	virtual uint64 GetRunTimes()const;			//执行次数
	virtual float  GetRunTimeOutRate()const;	//超时率
	virtual	uint64 GetRunElapsed()const;		//运行时间流逝总计
...
public:
	...
	uint64			_uRunTimeMax;		//单次运行峰值时间
	uint64			_uRunTimes;			//运行次数
	float			_fRunTimeOutRate;	//超时率
	uint64			_uRunElapsed;		//运行时间总计
