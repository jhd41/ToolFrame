...
struct XLogicUpdate
{
	...
	uint32	uRunTimes;		//单次执行次数
	uint8	uRunTimeOutRate;//单次执行的超时时间比率
	uint32	uRunElapsed;	//运行时间总计
	...
	//队列
	uint32	uQueueSize;		//队列长度
	uint32	uQueueProcess;	//队列处理量
	...
	uint32	nLogicCmd;		//逻辑队列数
	uint32	nNetCmd;		//网络队列数
	uint32	uSendBytes;		//发送字节数
	uint32	uRecvBytes;		//接收字节数
	...
	XLogicUpdate(){
		...
		uRunTimes = 0;		//单次执行 次数
		uRunTimeOutRate = 0;//单次执行 超时比率
		uRunElapsed = 0;	//运行时间总计
		uTimeSave = 0;		//最近保存时间
