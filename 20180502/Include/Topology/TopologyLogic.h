...
public:
	...
	bool SetRunOnceTimeAvg(uint64 uTimeRunOnce);						//单次执行平均时间
	bool SetRunOnceTimeMax(uint64 uTime);								//单次执行峰值时间
	bool SetRunTimes(uint64 uTimes);									//本次运行次数
	bool SetRunTimeOutRate(float fRate);								//设置超时率
	bool SetRunElapsed(uint64 uElapsed);								//运行时间流逝
...
private:
	...
	uint64					_uTimeRunOnceAvg;		//单次执行时间(毫秒 最大值65秒)
	uint64					_uTimeRunOnceMax;
	uint64					_uRunTimes;
	float					_fRunTimeOutRate;
	uint64					_uRunElapsed;
