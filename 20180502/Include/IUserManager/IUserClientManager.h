...
public:
	...
	virtual bool SyncTryServerRecved(const XServerArg& xServerArg, const IMsgStream& msgBase);
	virtual bool SyncClientConnected(uint uClientID);
	virtual bool SyncClientDisconnect(uint uClientID);
	virtual bool SyncTryClientRecved(uint uClientID, const IMsgStream& msgStream);
	virtual bool SyncTryClientRecved(uint uClientID, StreamBinaryPtr pMsgStream);
	virtual bool SyncClientUrlResponce(uint uClientID, CUrlRequestTopologyPtr pRequest);
