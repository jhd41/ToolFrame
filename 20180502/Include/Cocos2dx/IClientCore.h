...
//2.发布渠道:上架的渠道。 仅仅是提供我们上架的渠道，例如 是ios上架 还是移动MM上架 还是 和游戏上架 还是 游侠上架。主要还是为调用何种SDK服务
//3.推广渠道:销售的渠道。这表达的是 以何种方式 让玩家 安装上游戏的渠道。例如 是 斗鱼。推广渠道是非常重要的渠道，用户在通过该推广渠道的留存 支付 都要根据这个推广渠道计算
//4.属性渠道:用户的所属渠道。是推广渠道的分支，用户通过推广渠道安装上游戏后，该用户的任何行为就算作该推广渠道的行为，即使用户之后切换了渠道进行付费，仍然按照用户属性的渠道进行结款。
...
public:
	...
	CFIELD_STRING(PropertyChannelID);	//属性渠道
	CFIELD_UINT(GameID);				//游戏ID
