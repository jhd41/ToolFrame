...
std::string IApiTool::GetCallStackAll()
{
#if MACRO_TARGET_OS == MACRO_OS_IOS
...
std::string IApiTool::GetCallStack()
{
#if MACRO_TARGET_OS == MACRO_OS_IOS
...
std::string IApiTool::ToLanguageString(int eLanguage)
{
	switch (eLanguage)
	{
...
bool IApiTool::SystemSetTime(uint64 uTime)
{
#if MACRO_TARGET_OS == MACRO_OS_IOS
...
#if MACRO_TARGET_OS == MACRO_OS_WINDOWS
	return ApiWin::SystemSetTime(uTime);
#endif
