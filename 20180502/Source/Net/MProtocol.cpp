	...
	while(!timeOut.IsTimeOut()){
		...
		if (ptrSession->TryClearLogic(IDataSession::REQ_LOGIC_QUEUE)){
			if (ptrSession->TryClearLogic(IDataSession::REQ_CONNECTED)){
				DEBUG_POINT_TAG("MProtocol::OnProcessLogic OnConnected");
			...
			//接收
			if (ptrSession->IsConnected() && ptrSession->TryClearLogic(IDataSession::REQ_RECVED)){
				DEBUG_POINT_TAG("MProtocol::OnProcessLogic OnRecv");
				//如果能够回调
				if (ptrSession->IsCallBackRecv() && ptrSession->GetReceiver() && ptrSession->SwapRecvBuffer())
				{
					...
					{
						DEBUG_POINT_TAG("MProtocol::OnDelayRecv");
						...
						if (!ptrSession->RecvDataSplice(ptrBuffer, false))
						{
							...
							DEBUG_POINT_TAG("MProtocol::OnDelayRecv ClearDelayRecv");
			...
			if (ptrSession->TryClearLogic(IDataSession::REQ_CLOSED)) {
				...
				{
					DEBUG_POINT_TAG("MProtocol::OnProcessLogic OnClosed");
			...
			//如果Session还有效 就继续尝试
			if (!ptrSession->IsClosed() && ptrSession->IsLogicMarked()){

				DEBUG_POINT_TAG("MProtocol::OnProcessLogic PushFaild");
...
bool MProtocol::ProcessBalance(uint64 uProcessTime /*= 0*/, uint64 uNetResloved)
{
	...
	if (0 == uProcessTime)
	{
		bProcessed &= ProcessNet(uProcessTime);
		bProcessed &= ProcessLogic(uProcessTime);
		bProcessed &= ProcessNet(uProcessTime);
		return bProcessed;
	}
	...
	if (0 == uCountTotal){
		...
		bProcessed &= ProcessNet(uProcessTimeNet);
		bProcessed &= ProcessLogic(uProcessTimeLogic);
		bProcessed &= ProcessNet(uProcessTimeNet);
		return bProcessed;
	}
