...
bool MSystem::RunOnce( uint64 uTime )
{
	...
	//处理事件
	bool bTimeOut1 = !_mEventTrigger.ProcessEvent(timeout.TimeRemaining());

	//处理URL
	_xUrlProcessor.ProcessOnce();

	//处理时间
	bool bTimeOut2 = !_vQueueTime.ProcessTestTimeOut(ToolFrame::GetNowTimeMill(), timeout.TimeRemaining());
	bool bTimeOut3 = !_vQueueTime.ProcessTimeOut(timeout.TimeRemaining());
	
	//处理协议
	bool bTimeOut4 = !MProtocol::Singleton().ProcessBalance(uTime);
	...
	bool bTimeOut5 = timeout.IsTimeOut();
	...
	return bTimeOut1 || bTimeOut2 || bTimeOut3 || bTimeOut4 || bTimeOut5;
}
