...
bool CTopologyLogic::SetRunOnceTimeAvg( uint64 uTimeRunOnce )
{
	_uTimeRunOnceAvg = uTimeRunOnce;
	return true;
}
...
bool CTopologyLogic::SetRunTimes(uint64 uTimes)
{
	_uRunTimes = uTimes;
	return true;
}

bool CTopologyLogic::SetRunTimeOutRate(float fRate)
{
	_fRunTimeOutRate = fRate;
	return true;
}

bool CTopologyLogic::SetRunElapsed(uint64 uElapsed)
{
	_uRunElapsed = uElapsed;
	return true;
}
...
bool CTopologyLogic::GetUpdateData( XLogicUpdate& xUpdate )
{
	...
	uint64 uProcessTotal,uProcessMax=0,uProcessAvg = 0; uint64 uProcessTimes = 0;
	...
	xUpdate.uTimeRunOnce	= (uint16)_uTimeRunOnceAvg;
	xUpdate.uTimeRunOnceMax	= (uint16)_uTimeRunOnceMax;
	xUpdate.uRunTimes		= (uint32)_uRunTimes;
	xUpdate.uRunTimeOutRate = (uint8)(_fRunTimeOutRate*100);
	xUpdate.uRunElapsed		= (uint32)_uRunElapsed;
