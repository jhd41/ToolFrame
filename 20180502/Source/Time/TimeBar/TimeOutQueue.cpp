...
bool CTimeOutQueue::ProcessTestTimeOut( uint64 uTimeNow,uint64 uTimeProcess )
{
	DEBUG_FUN();
	...
	bProcessed &= ProcessTestTimeOutPoint(uTimeNow,tTimeOut);
	bProcessed &= ProcessTestTimeOutAfter(uTimeNow,tTimeOut);
	return bProcessed;
}

bool CTimeOutQueue::ProcessTestTimeOut( uint64 uTimeProcess )
{
	return ProcessTestTimeOut(GetModeTimeNow(),uTimeProcess);
}
...
{
	return ClearTimeOutTimeBarPoint(ToolFrame::GetNowTimeMill());
}

bool CTimeOutQueue::ProcessTestTimeOutAfter( uint64 uTimeNow,const CTimeOut& tTimeOut )
{
	...
	if (ToolFrame::IsEmpty(_vTimeBarAfter.GetMap()))return true;
...
bool CTimeOutQueue::ProcessTestTimeOutPoint( uint64 uTimeNow,const CTimeOut& tTimeOut )
{
	if (IsPausing())return true;
