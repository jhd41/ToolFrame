...
std::string CMySqlResults::GetFieldName(uint uIndex) const//获取字段名称
{
	if (uIndex >= GetFieldCount())return "";
	MYSQL_FIELD* pFeild = mysql_fetch_field_direct(_pSqlRes, uIndex);
	if (pFeild && pFeild->name)
		return pFeild->name;
