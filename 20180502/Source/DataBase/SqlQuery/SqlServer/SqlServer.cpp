...
size_t CSqlServer::GetQueryCount() const
{
	return MMySql::Singleton().GetQueryCount();
}

bool CSqlServer::IsEmpty() const
{
	return MMySql::Singleton().IsEmpty();
}

size_t CSqlServer::ClearProcessCount()
{
	return MMySql::Singleton().ClearProcessCount();
}
...
size_t CSqlServer::GetClientCount() const
{
	return _vUserMgr.GetClientCount();
}

size_t CSqlServer::GetAssCount() const
{
...
bool CSqlServer::OnSqlExecuted(IMySqlQuery* pSqlQuery, CMySqlResults& results)
{
	...
	if (!pBuffer)
	{
		LOG_CLIENT_ERR_MSG("ERR_SQL_DATA_SIZE_MAX:"<<pSqlQuery->GetSql());

		MsgSqlResponse msgErr;
		msgErr.uSqlID = pSqlQuery->GetID();
		msgErr.uErr = ERR_SQL_DATA_SIZE_MAX;
		_vUserMgr.SyncSendClient(pSqlQuery->GetClientID(), msgErr);
		return true;
	}
	_vUserMgr.SyncSendClient(pSqlQuery->GetClientID(), pBuffer);
	return true;
}
