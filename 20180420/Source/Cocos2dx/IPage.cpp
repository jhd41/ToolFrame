...
bool IPage::OnTouchEventEnd(Ref *pSender)
{
	return false;
}
...
{	
...

	{
		...
		//统计
		ToolFrame::MAnalytics::Singleton().OnEventBegin(MAKE_STRING(ToolFrame::GetTypeName(this)));
	}
...
void IPage::touchEvent( Ref *pSender, TouchEventType type )
{
	...
	if (TouchEventType::TOUCH_EVENT_ENDED == type)
		ToolFrame::MAnalytics::Singleton().OnEvent("OnTouchEventEnd","NodeName",MAKE_STRING(ToolFrame::GetTypeName(this) << "_" << ApiCocos2dx::GetName(pSender)));

	OnTouchEvent(pSender,type);
	if (TouchEventType::TOUCH_EVENT_ENDED == type)
		OnTouchEventEnd(pSender);
}
...
bool IPage::Destory()
{
	...
	{
		//统计
		ToolFrame::MAnalytics::Singleton().OnEventEnd(MAKE_STRING(ToolFrame::GetTypeName(this)));
	}
