...
int ApiCUrl::ApiDownLoadProgress( void *ptr, double fTotalDownload, double fNowDownloaded, double fTotalToUpLoad, double fNowUpLoaded )
{
	HCUrl* hHandler = (HCUrl*)ptr;
	if (ptr)
		hHandler->OnDownLoadProgress(fTotalDownload,fNowDownloaded);
	return 0;
}
...
bool ApiCUrl::InitGlobal()
{
	static CURLcode eErr = curl_global_init(CURL_GLOBAL_ALL);
	return true;
}
