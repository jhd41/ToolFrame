	...
	case CLIENT_TYPE_CONTROL:
		{
			...
			case MSG_CONTROL_PHYSICAL_SET_TIME:
				{
					MsgControlPhysicalSetTime msg(msgStream);
					if (!msg.Load())return false;
					...
					_hHandler->OnControlSetTime(pPhysical->uPhysicalID, msg.uMillTime);
					
					MsgCenterNtf msgReq;
					msgReq.uType = MsgCenterNtf::TYPE_PHYSICAL_SET_TIME;
					msgReq.uMillTime = msg.uMillTime;
					Send(pPhysical->uClientID, msgReq);
					return true;
				}
				break;
