...
std::string ToolFrame::RemoveUrlArg(const std::string& sUrl)
{
	if (sUrl.empty())return sUrl;

	SSIZE_TYPE uOffSet1 = sUrl.find("?");
	SSIZE_TYPE uOffSet2 = sUrl.find("#");

	SSIZE_TYPE uOffSet = ToolFrame::Min(uOffSet1, uOffSet2);

	if (std::string::npos == uOffSet)
		return sUrl;

	return sUrl.substr(0, uOffSet);
}
...
size_t ToolFrame::GetFileLength(const std::ifstream& fStreamIn)
{
	...
	return (size_t)uFileLen;
}

size_t ToolFrame::GetFileLength( const std::fstream& fStreamIn )
{
...
size_t ToolFrame::GetStreamLength( const std::ifstream& fStreamIn )
{
	std::ifstream& fStream = ToolFrame::ConstCast(fStreamIn);
	...
	return (size_t)(uFileLen - uPos);
}

size_t ToolFrame::GetStreamLength( const std::fstream& fStreamIn )
{
	std::fstream& fStream = ToolFrame::ConstCast(fStreamIn);
...
bool ToolFrame::SplitStringFirst( VectorString& vReturn,const std::string& sSrc,const std::string& sSP,bool bFindFront /*= true*/ )
{
	if (sSrc.empty())return false;

	SSIZE_TYPE uOffSet = bFindFront ? sSrc.find(sSP):sSrc.rfind(sSP);
	if (std::string::npos == uOffSet)return false;
...
std::string ToolFrame::SplitStringFirst(const std::string& sSrc, const std::string& sSP, bool bFindFront /*= true*/, bool bHead /*= true*/)
{
	if (sSrc.empty())return sSrc;

	SSIZE_TYPE uOffSet = bFindFront ? sSrc.find(sSP) : sSrc.rfind(sSP);
	if (std::string::npos == uOffSet)return "";

	return bHead ? sSrc.substr(0, uOffSet) : sSrc.substr(uOffSet + sSP.length(), sSrc.length() - uOffSet);
}
