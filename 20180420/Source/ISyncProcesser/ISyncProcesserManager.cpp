...
bool ISyncProcesserManager::SyncCalState(uint& uTotal, uint& uVaild, uint& uNormal, uint& uQueue, uint& uDelete, uint& uPerpairing, uint& uCache,uint& uOperateQueue, uint& uDelayOperateQueue) const
{
	uQueue = 0;
	uDelete = 0;
	uVaild = 0;
	...
	foreach(itr, _vProcesser) {
		...
		uOperateQueue += pProcesser->GetOperateQueue().GetOperateSize();
		uDelayOperateQueue += pProcesser->GetOperateQueue().GetOperateDelaySize();
