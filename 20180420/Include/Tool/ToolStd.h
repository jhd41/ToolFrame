...
//  typename std::vector<T>::iterator	EraseByItr(std::vector<T>& vSrc,typename std::vector<T>::const_iterator& itr);
	...
	//字符串 - 分离 遇到第一个分隔符就分离
	bool			SplitStringFirst(VectorString& vReturn,const std::string& sSrc,const std::string& sSP,bool bFindFront = true);//（搜索起点）
	std::string		SplitStringFirst(const std::string& sSrc, const std::string& sSP, bool bFindFront = true, bool bHead = true);//（搜索起点，返回 头部 还是尾部）
	...
	std::string RemoveUrlArg(const std::string& sUrl);
	...
	size_t		GetFileLength(const std::ifstream& fStream);	//获取文件总长度
	size_t		GetFileLength(const std::fstream& fStream);	//获取文件总长度
	size_t		GetStreamLength(const std::ifstream& fStream);//获取流还未读取的长度
	size_t		GetStreamLength(const std::fstream& fStream);	//获取流还未读取的长度
