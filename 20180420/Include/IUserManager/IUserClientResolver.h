...
public:
	...
	virtual bool OnClientRecv(StreamBinaryPtr pMsgStream) { return false; }
	virtual bool OnClientRecvErr(const IMsgStream& msgStream);						//当客户端协议处理有误时
	virtual bool OnClientRecvErr(StreamBinaryPtr pMsgStream);							//当客户端协议处理有误时
	
	virtual bool OnClientUrlResponce(CUrlRequestTopologyPtr pRequest) { return false; }
	virtual bool OnClientUrlResponceErr(CUrlRequestTopologyPtr pRequest);
