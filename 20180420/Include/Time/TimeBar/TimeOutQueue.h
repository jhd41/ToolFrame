...
public:
	...
	//系统调用
public:
	bool		TestTimeOut(uint64 uTimeProcess);				//计算是否超时以当前时间作为参考时间(返回是否超时)
	bool		TestTimeOut(uint64 uTimeNow,uint64 uTimeProcess);//计算是否超时(返回是否超时)
	bool		ProcessTimeOut(uint64 uTimeProcess);					//处理已经超时的对象(返回是否超时)
...
private:
	...
	bool		TestTimeOutAfter( uint64 uTimeNow,const CTimeOut& tTimeOut );
	bool		TestTimeOutPoint( uint64 uTimeNow,const CTimeOut& tTimeOut );
